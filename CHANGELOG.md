...
- 设置页支持新增、编辑和删除自定义 Agent 工具；自定义工具仅作为全局 Skills 目录目标，图标复制到 Workbench 本地 `tool-icons` 目录管理。

### Changed

- 自定义工具弹窗隐藏内部标识，新增时由后端根据工具名称自动生成唯一标识，并在弹窗内展示表单错误。
- 软件更新弹窗的更新说明改为分点展示，便于快速扫描每项变更。
- 项目打开方式的终端命令支持在命令字段中填写内联参数，例如 `deveco -c --skip-agreement`。
- 项目打开方式弹窗优化参数和工作目录提示，减少 `{projectPath}` 重复填写的误导。

### Fixed

- 修复自定义工具本地图标可能显示为坏图的问题，开启 Tauri asset protocol 并在图标加载失败时回退为字母图标。
