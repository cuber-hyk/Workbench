...
- Skills 当前支持 `skills.sh` 技能市场和 public GitHub 直连导入两类远程来源；`skills.sh` 安装和更新通过隔离临时目录调用官方 `npx skills add` 提取内容，GitHub 直连导入和分支来源更新在配置 Token 后优先使用 GitHub API，未配置时使用本机 Git 浅克隆。
...
- Skills：扫描、分类、导入、skills.sh 市场安装/卸载、远程来源更新、GitHub Token 配置、全局启用、项目启用、冲突解决和删除已接入真实后端。
...
- GitHub Token 仅用于 public GitHub Skill 导入和 GitHub 分支来源更新的 API 优先路径；保存后不在 UI、诊断复制信息、toast 或错误中回显明文。
