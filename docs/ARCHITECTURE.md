...
Workbench App 是一个本地优先的桌面应用。第一阶段已经完成项目管理、Skills 管理、资源 Radar 和设置四个基础模块；当前进入进阶开发阶段，在保持本地优先和清晰边界的前提下扩展 Agent 工具、Skills 来源、资源导入和自动化能力。
...
- 发现已注册工具全局目录中的既有 Skills，并由用户显式导入统一根目录。
...
- 外部工具目录只作为发现来源和启用目标，不成为真实来源；发现过程只读，不创建、不复制、不写数据库。
...
### 7.6.1 发现外部工具目录 Skills

流程：

1. 用户在 Skills 页面点击“发现已有工具 Skills”。
2. 后端只读扫描已注册工具的全局 Skills 目录，包括内置工具和自定义工具。
3. 后端只检查一级子目录中的 `SKILL.md`，不递归扫描项目级目录或嵌套仓库内容。
4. 后端按 Skill 目录名合并多个工具来源；同名相同内容显示为同一候选，同名不同内容显示冲突。
5. 用户显式导入可导入项后，后端复制选定来源到当前统一 Skills 根目录。
6. 导入后默认不启用到任何 Agent 工具目录。

边界：

- 发现过程不创建目录、不复制文件、不写数据库。
- 当前统一根目录已存在同名相同内容时跳过。
- 当前统一根目录已存在同名不同内容时返回冲突，不覆盖。
