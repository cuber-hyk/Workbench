...
- `src-tauri/src/skills.rs`：Skills 扫描、导入、skills.sh 市场安装与更新、启用、冲突、删除、自定义工具目标和 SQLite 逻辑。
- `docs/capabilities/skills-management.md`：Skills 当前能力、skills.sh 来源、自定义工具目标、分类与筛选语义、数据所有权、同步边界和验证。
- `docs/adr/2026-06-18-skill-categories-table.md`：Skills 分类使用独立分类表和 `category_id` 的长期决策。
- `docs/adr/2026-06-19-skills-sh-self-managed-install.md`：Workbench 自管 skills.sh Skill 安装与更新，含 skill_sources 表、内容 hash 检测和备份策略的决策。
