...
- 为每个 Skill 维护一个 Workbench 内分类；空分类统一保存为“未分类”。
...
- 在 Skills 列表中切换 Codex、Claude Code 和 OpenCode 的全局启用状态。
- 在 Skill 详情中按项目批量启用或停用支持项目级 Skills 的工具。
...
- 前端测试覆盖工具筛选、项目筛选、组合筛选、分类选择、新建分类和详情区去重。
- 后端 Rust 测试覆盖扫描、导入、启用、停用、冲突检测、备份和删除边界。
