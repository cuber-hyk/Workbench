...
“软件更新”弹窗展示当前版本、最新版本、发布时间、更新分类统计和结构化更新说明。Release Notes 仍来自 updater 返回的文本，但前端只解析版本标题、分类标题和列表项，并按新增、优化、修复、安全等分组展示，避免把 Markdown 原文直接暴露给用户。用户确认后才能下载并安装更新；下载中展示进度，安装完成后需要用户点击重启入口完成更新。
