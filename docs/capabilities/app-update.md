...
- GitHub Releases notes 读取和旧安装检测：`src-tauri/src/app_update.rs`
...
“软件更新”弹窗展示当前版本、最新版本、发布时间、更新分类统计和结构化更新说明。检查到新版本后，Workbench 会在保持 Tauri updater `latest.json` 作为下载和签名来源的前提下，额外读取 GitHub Releases notes，并展示当前版本之后到最新版本之间的累计变更。前端只解析版本标题、分类标题和列表项，并按新增、优化、修复、安全等分组展示，避免把 Markdown 原文直接暴露给用户。用户确认后才能下载并安装更新；下载中展示进度，安装完成后需要用户点击重启入口完成更新。

如果 GitHub Releases notes 读取失败，更新弹窗回退展示 updater 返回的最新版本说明；更新下载和安装流程不受影响。

Workbench 从旧版 `Workbench App / workbench-app.exe` 迁移到当前 `Workbench / workbench.exe` 后，启动时会检查 Windows 旧卸载项和仍指向 `workbench-app.exe` 的桌面/开始菜单快捷方式。发现旧安装时，应用通过 toast 提示用户进入更新弹窗处理；弹窗内可以删除明确指向旧程序的旧快捷方式，也可以打开旧版卸载程序。Workbench 不会静默删除旧安装目录，避免误删用户自定义安装目录中的其他文件。
