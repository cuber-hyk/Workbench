...
|---|---|---|---|---|
| APP-SPLIT-0 | done | `task/20260621-split-app-tsx` | Commit the current Projects/launch extraction and focused plan after review approval. | Commit `1ad86c0`; observed: `pnpm build`, `pnpm test`, `git diff --check`, docs validation |
| APP-SPLIT-1 | done | `task/20260621-split-app-settings` | Move `SettingsView`, `projectOpenProfileSummary`, and `closeBehaviorLabel` into `src/views/settings/SettingsView.tsx` and `src/views/settings/settingsFormatters.ts`. Keep settings dialogs in `App.tsx` for this branch. | `pnpm build`; `pnpm test`; `rg` confirms no duplicate `SettingsView` implementation in `App.tsx` |
| APP-SPLIT-2 | done | `task/20260621-split-app-dialogs` | Move leaf dialogs into `src/components/dialogs/` by feature group: project, settings, skills. Keep callback wiring in `WorkbenchApp`. | `pnpm build`; `pnpm test`; focus review dialog props and modal footer behavior |
